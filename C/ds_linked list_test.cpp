...
void add()
{
	...
	else
	{
		...
		for(int i=0;i<MAXLEN;i++)
		{
			...
			{
				table[i].data = data;//1.2 把資料放入目前找到的空位置 
				if(table[next].next != -1)//1.3 判斷欲新增的位置之前有無串列，若無則進入 否則另外處理 
				{
					table[i].next = table[next].next;//1.4 將指定位置的指標，轉指向給新資料
					table[next].next = i; //1.5 將目前位置，轉指向給指定位置 
				}
				else
				{
					table[next].next = i;//1.6 將目前位置，轉指向給指定位置 
				}
				//插入資料之後 
				break;
			}	
